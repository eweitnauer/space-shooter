...
JoinDialogAssistant = Class.create({
	...
	setup : function(widget) {
		this.widget = widget;
		this.controller.setupWidget("session_code", { textCase: Mojo.Widget.steModeLowerCase, hintText: '... type here' }, { });
...
MainAssistant.prototype.join_pressed = function(evt) {
  Mojo.Log.info('Join pressed.');
...
MainAssistant.prototype.on_connect = function(item) {
  ...
  Mojo.Log.info('Connected to ' + item.addr + '!');
  if (item.timerId) {
...
MainAssistant.prototype.on_disconnect = function(item) {
  ...
  item.spinning = false;
  this.controller.modelChanged(this.connectionsModel, this);
  Mojo.Log.info('Disconnected from ' + item.addr + '!');
  if (item.timerId) {
    clearTimeout(item.timerId);
    item.timerId = null;
  }
...
MainAssistant.prototype.on_conn_err = function(item) {
  ...
  item.spinning = false;
  this.controller.modelChanged(this.connectionsModel, this);
  Mojo.Log.info('Could not connect / disconnect from ' + item.addr + '!');
}
...
MainAssistant.prototype.connection_list_tapped = function(evt) {
  ...
  if (typeof(evt.item.socket) == 'undefined') {
    evt.item.socket = io.connect(evt.item.addr);
...
//                    this.create_pressed.bindAsEventListener(this));
  ...
  Mojo.Event.listen(this.controller.get("btnJoin"),
                    Mojo.Event.tap,
                    this.join_pressed.bindAsEventListener(this));

  this.controller.setupWidget("connection-list",
      this.connectionsAttr = {
        itemTemplate: "main/item-template"
      },
      this.connectionsModel = {
        listTitle: "connections",
        items: [{addr: 'http://192.168.0.102:9888', label: '', spinning: false},
                {addr: 'http://phigames.com:9888', label: '', spinning: false}]
      }
  );
