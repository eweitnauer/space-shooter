...
exports['basics'] = function(test) {
  ...
  test.fequal(a.x, -1 * l);
  test.fequal(a.y, -2 * l);
  test.fequal(a_orig.normalized().x, a.x);
  test.fequal(a_orig.normalized().y, a.y);
  a = a_orig.clone();
  test.deepEqual(new Point(0,0), (new Point(0,0)).normalized());
  ...
  test.equal(a.scaled(2).x, -2);
  test.equal(a.scaled(2).y, -4);
  test.done();
}
